...
function loadSettings() {
    ...
    const savedFixedHeaderEnabled = localStorage.getItem('fixedHeaderEnabled');
    ...
    return {
        ...
        fixedHeaderEnabled: savedFixedHeaderEnabled !== null ? savedFixedHeaderEnabled === 'true' : true
    };
}
...
function saveSettings() {
    ...
    localStorage.setItem('fixedHeaderEnabled', fixedHeaderEnabled);
}
...
let fixedHeaderEnabled = settings.fixedHeaderEnabled;
...
if (!fixedHeaderEnabled) {
    document.body.classList.add('fixed-header-disabled');
}
...
const fixedHeaderToggle = document.getElementById('fixedHeaderToggle');
...
// Ενημέρωση του fixed header toggle με την αποθηκευμένη τιμή
if (fixedHeaderToggle) {
    if (fixedHeaderEnabled) {
        fixedHeaderToggle.classList.add('active');
    }
    
    fixedHeaderToggle.addEventListener('click', () => {
        fixedHeaderEnabled = !fixedHeaderEnabled;
        fixedHeaderToggle.classList.toggle('active');
        
        if (!fixedHeaderEnabled) {
            document.body.classList.add('fixed-header-disabled');
        } else {
            document.body.classList.remove('fixed-header-disabled');
        }
