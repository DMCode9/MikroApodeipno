...
// Μετατροπή πολυτονικού σε μονοτονικό για καλύτερη ανάγνωση από το TTS
function toMonotonic(text) {
    let nfd = text.normalize('NFD');
    // Mετατροπή βαρείας και περισπωμένης σε οξεία
    nfd = nfd.replace(/[\u0300\u0342]/g, '\u0301');
    // Αφαίρεση πνευμάτων (ψιλή, δασεία), κορωνίδας, υπογεγραμμένης, μακρού, βραχέος
    nfd = nfd.replace(/[\u0313\u0314\u0343\u0345\u0304\u0306]/g, '');
    // Καθαρισμός διπλών τόνων αν προκύψουν
    nfd = nfd.replace(/\u0301+/g, '\u0301');
    return nfd.normalize('NFC');
}

// Επέκταση των επαναλήψεων για την ανάγνωση
function expandRepetitions(text) {
    // Κύριε ελέησον (3, 12, 40)
    ...
    // Αγίων Αγιώτατον Λόγον (από Χαιρετισμούς)
    text = text.replace(/Αγίων Αγιώτατον Λόγον(?:\s+\(εκ γ’\)|\s+\(εκ γ'\))?\s*\(3\)/gi, () => {
        return Array(3).fill("Αγίων Αγιώτατον Λόγον.").join(" ");
    });

    // Αφαίρεση υπολειπόμενων παρενθέσεων επανάληψης ώστε να μην τις διαβάσει το TTS
    text = text.replace(/(?:\(εκ γ’\)|\(εκ γ'\)|γ’|γ'|ιβ’|ιβ'|μ’|μ')?\s*\(\d+\)/g, "");

    return text;
}

const ttsPlayIcon = `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-volume-2"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/><path d="M15.54 8.46a5 5 0 0 1 0 7.07"/><path d="M19.07 4.93a10 10 0 0 1 0 14.14"/></svg>`;
const ttsStopIcon = `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-square"><rect width="12" height="12" x="6" y="6" rx="2" fill="currentColor" stroke="none"/></svg>`;
...
if (ttsBtn) {
    ttsBtn.addEventListener('click', () => {
        if (isSpeaking || window.speechSynthesis.speaking) {
            ...
            isSpeaking = false;
            ttsBtn.querySelector('.icon').innerHTML = ttsPlayIcon;
            return;
        }

        let fullText = '';
        
        // If theotokos overlay is active, read the chairetismoi
        const theotokosOverlay = document.getElementById('theotokosOverlay');
        if (theotokosOverlay && theotokosOverlay.classList.contains('active')) {
            const paragraphs = document.querySelectorAll('.chairetismoi-paragraph');
            paragraphs.forEach(p => {
            ...
            paragraphs.forEach(p => {
                // If side-by-side mode is active, get original text
                const originalTextDiv = p.querySelector('.original-text');
                if (originalTextDiv) {
                    const textNodes = Array.from(originalTextDiv.querySelectorAll('p')).map(el => el.textContent);
                    fullText += textNodes.join(' ') + ' ';
                } else {
                    // Regular paragraph
                    const pElements = Array.from(p.querySelectorAll('p')).map(el => el.textContent);
                    fullText += pElements.join(' ') + ' ';
                }
            });
        }
        ...
        if (fullText) {
            // Μετατροπή σε μονοτονικό και επέκταση των επαναλήψεων
            let monotonicText = toMonotonic(fullText);
            monotonicText = expandRepetitions(monotonicText);
            
            const utterance = new SpeechSynthesisUtterance(monotonicText);
            utterance.lang = 'el-GR';
            utterance.rate = 0.85; // A bit slower, solemn reading
            ...
            isSpeaking = true;
            ttsBtn.querySelector('.icon').innerHTML = ttsStopIcon;
        }
    });
}
